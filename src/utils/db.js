  ...
  return events
    ...
    .map(event => {
      ...
      return {
        ...
        비고: event.note ?? '',
        줄기직경mm: valueOf(byType, '줄기직경') ?? valueOf(byType, '가지직경'),
        가지직경mm: valueOf(byType, '가지직경') ?? valueOf(byType, '줄기직경'),
        가지직경_캘리퍼스mm: valueOf(byType, '가지직경_캘리퍼스기준') ?? valueOf(byType, '줄기직경_캘리퍼스기준'),
        근원직경mm: valueOf(byType, '근원직경'),
        근원직경_캘리퍼스mm: valueOf(byType, '근원직경_캘리퍼스기준'),
        캘리퍼스줄기직경mm: valueOf(byType, '줄기직경_캘리퍼스기준') ?? valueOf(byType, '가지직경_캘리퍼스기준'),
...
export async function downloadPhoneBackupJson(meta = {}) {
  const db = await openDB()
  ...
  downloadTextFile(
    `${stamp.slice(0, 8)}_plum-measure-${researcher}_${stamp.slice(9)}.json`,
